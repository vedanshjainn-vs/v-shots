...
void main() {
  final builder = TasteProfileBuilder(
    config: RecommendationConfig.defaultConfig,
  );
  ...
  group('artist affinity', () {
    ...
    test('affinity decays with age (recency-weighted)', () {
      ...
      expect(
        recent.artistAffinity['D']!,
        greaterThan(old.artistAffinity['D']!),
      );
    });
  });
  ...
  group('skip penalty', () {
    ...
    test(
      'skip penalty decays with age — does not permanently punish an artist',
      () {
        final now = DateTime.now();
        final recentSkip = builder.build(
          events: [
            SignalEvent(
              type: SignalType.skip,
              timestamp: now,
        ...
        expect(
          recentSkip.artistSkipPenalty['G']!,

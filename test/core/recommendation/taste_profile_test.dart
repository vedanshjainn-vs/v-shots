...
void main() {
  final builder = TasteProfileBuilder(config: RecommendationConfig.defaultConfig);
  ...
  group('artist affinity', () {
    ...
    test('completed track contributes more than a bare play', () {
      ...
      final playOnly = builder.build(events: [
        SignalEvent(type: SignalType.play, timestamp: now, artist: 'A', trackId: 't1'),
      ]);
      final completed = builder.build(events: [
        SignalEvent(type: SignalType.completed, timestamp: now, artist: 'A', trackId: 't1'),
      ]);
      expect(completed.artistAffinity['A']!, greaterThan(playOnly.artistAffinity['A']!));
    });
    ...
    test('unlike is a mild negative, not punitive', () {
      final profile = builder.build(events: [
        SignalEvent(type: SignalType.unlike, timestamp: DateTime.now(), artist: 'C', trackId: 't3'),
      ]);
    ...
    test('affinity decays with age (recency-weighted)', () {
      ...
      final recent = builder.build(events: [
        SignalEvent(type: SignalType.completed, timestamp: now, artist: 'D', trackId: 't4'),
      ]);
      ...
      expect(recent.artistAffinity['D']!, greaterThan(old.artistAffinity['D']!));
    });
  });
  ...
  group('skip penalty', () {
    ...
    test('late skip (near end of track) produces a smaller penalty than an immediate skip', () {
      ...
      final immediate = builder.build(events: [
        SignalEvent(type: SignalType.skip, timestamp: now, artist: 'F', trackId: 't6', value: 2.0),
      ]);
      final late = builder.build(events: [
        SignalEvent(type: SignalType.skip, timestamp: now, artist: 'F', trackId: 't6', value: 180.0),
      ]);
      expect(immediate.artistSkipPenalty['F']!, greaterThan(late.artistSkipPenalty['F']!));
    });

    test('skip penalty decays with age — does not permanently punish an artist', () {
      final now = DateTime.now();
      final recentSkip = builder.build(events: [
        SignalEvent(type: SignalType.skip, timestamp: now, artist: 'G', trackId: 't7', value: 2.0),
      ]);
      ...
      expect(recentSkip.artistSkipPenalty['G']!, greaterThan(oldSkip.artistSkipPenalty['G']!));
      // Old skip should have decayed toward (but not necessarily to)
      // zero, not stayed at full strength.
      expect(oldSkip.artistSkipPenalty['G']!, lessThan(recentSkip.artistSkipPenalty['G']!));
    });

    test('one skip does not appear in artistAffinity — tracked separately', () {
      final profile = builder.build(events: [
        SignalEvent(type: SignalType.skip, timestamp: DateTime.now(), artist: 'H', trackId: 't8', value: 2.0),
      ]);
  ...
  group('cold start', () {
    ...
    test('fewer than 3 signals is not enough for personalization', () {
      final profile = builder.build(events: [
        SignalEvent(type: SignalType.play, timestamp: DateTime.now(), artist: 'X', trackId: 't10'),
        SignalEvent(type: SignalType.play, timestamp: DateTime.now(), artist: 'Y', trackId: 't11'),
      ]);
      expect(profile.hasEnoughHistoryForPersonalization, isFalse);
    });
  });
  ...
  group('topArtists / topGenres ordering', () {
    test('topArtists is sorted by affinity descending', () {
      final now = DateTime.now();
      final profile = builder.build(events: [
        SignalEvent(type: SignalType.play, timestamp: now, artist: 'Low', trackId: 't12'),
        SignalEvent(type: SignalType.like, timestamp: now, artist: 'High', trackId: 't13'),
        SignalEvent(type: SignalType.like, timestamp: now, artist: 'High', trackId: 't13'),
      ]);

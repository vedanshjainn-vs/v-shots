...
void main() {
  group('isIrrelevantContent', () {
    test('rejects gaming / news / sports / podcast / tutorial', () {
      expect(
        isIrrelevantContent('My PUBG Gameplay Highlights', 'GamerX'),
        isTrue,
      );
      expect(isIrrelevantContent('Daily News Update', 'News24'), isTrue);
      expect(
        isIrrelevantContent('Cricket Match Highlights', 'SportsTV'),
        isTrue,
      );
      expect(isIrrelevantContent('The Daily Podcast ep 9', 'Host'), isTrue);
      expect(
        isIrrelevantContent('How to make a beat (FL Tutorial)', 'Tutor'),
        isTrue,
      );
    });
    ...
    test('rejects reactions / vlogs / explanations', () {
      ...
      expect(
        isIrrelevantContent('Movie Ending Explained', 'ExplainIt'),
        isTrue,
      );
    });
    ...
    test('accepts real music titles', () {
      expect(
        isIrrelevantContent('Tum Hi Ho (Official Video)', 'Arijit Singh'),
        isFalse,
      );
  ...
  group('filterRelevantTracks', () {
    ...
    test(
      'relaxed fallback: returns original when filter empties the batch',
      () {

...
bool isAdPage(int page, bool adsEnabled) {
  if (!adsEnabled || page == 0) return false;
...
void main() {
  group('Discovery swipeable native ad page', () {
    ...
    test('ads disabled keeps PageView one-page-per-song', () {
    ...
    test('ad pages are inserted after every configured organic cadence', () {
    ...
    test('every non-ad page maps to a valid organic song', () {
    ...
    test('song index round-trips through the PageView mapping', () {
    ...
    test('ad page is a real PageView slot, not a modal trigger', () {
      // The implementation contract is structural: an ad slot increases the
      // PageView item count and therefore can be swiped away like any other
      // child. Modal interstitials do not participate in PageView itemCount.

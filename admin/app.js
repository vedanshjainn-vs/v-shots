    ...
    <div class="card">
      <div class="card-header">
        <div>
          ...
          <div class="card-subtitle">Control what appears on the V Shots Home screen. Changes are live after publishing.</div>
        </div>
        <div class="btn-group">
      ...
      <div style="margin-bottom:16px;padding:12px;background:var(--surface2);border-radius:8px;font-size:13px;color:var(--text2)">
        <strong>How it works:</strong> Sections are rendered top-to-bottom on the Home screen. 
        "Continue Listening" is always first (built-in). Add your sections below. 
        Use <code>youtube_search</code> for search-based sections, or add individual items.
      </div>
...
function renderProviders(el) {
  el.innerHTML = `
    <div class="stats-grid">
      ...
      <div class="stat-card"><div class="stat-label">JioSaavn</div><div class="stat-value" style="color:var(--green)">Web Playback</div><div class="stat-change">Official webpage only</div></div>
    </div>
    <div class="card">
      <div class="card-title">🔌 Provider Configuration</div>
      <p style="color:var(--text2);margin-top:8px;font-size:14px">Provider settings are managed through Supabase. Configure playback providers per-song in the Home CMS.</p>
      <div style="margin-top:16px;padding:12px;background:var(--surface2);border-radius:8px;font-size:13px">
        <strong>JioSaavn URL Helper:</strong><br>
        1. Open <a href="https://www.jiosaavn.com" target="_blank">JioSaavn</a><br>
        2. Search the song<br>
        3. Open the exact song page<br>
        4. Copy the page URL (e.g. <code>https://www.jiosaavn.com/song/song-name/ID</code>)<br>
        5. Paste it in the song's JioSaavn URL field
      </div>
    </div>
  `;
}
...
function renderFeatureFlags(el) {
  const flags = [
    ...
    { name: 'ENABLE_YOUTUBE_WEB_PLAYBACK', desc: 'YouTube WebView playback', active: true },
    ...
    { name: 'ENABLE_HOME_CMS', desc: 'Remote Home page CMS', active: true },
    { name: 'ENABLE_REMOTE_HOME', desc: 'Fetch Home config from Supabase', active: true },
    { name: 'ENABLE_JIOSAAVN', desc: 'JioSaavn metadata provider', active: false },
    { name: 'ENABLE_JIOSAAVN_WEB_PLAYBACK', desc: 'JioSaavn WebView playback', active: false },
  ];

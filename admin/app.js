...
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imp6eHR4cWp...';
...
async function load(){
  try {
    ...
    render(); status(`CMS connection failed: ${e.message || e}`, true);
  }
}

function render(){
  $('sections').innerHTML='';
  $('auth').innerHTML = state.admin
    ? '<span class="auth-user">Admin connected</span><button id="signout">Sign out</button>'
  ...
  state.sections.forEach((s,i)=>{
    ...
    d.innerHTML=`
      <input class="title" value="${esc(s.title)}" placeholder="Section title">
      <input class="subtitle" value="${esc(s.subtitle || '')}" placeholder="Subtitle (optional)">
      <select class="source">
        <option value="youtube_search">YouTube Search</option>
        <option value="youtube_playlist">YouTube Playlist</option>
        <option value="youtube_channel">YouTube Channel</option>
        <option value="youtube_manual">Manual Videos</option>
        <option value="youtube_trending">YouTube Trending</option>
      </select>
      <input class="value" value="${esc(s.source_value || '')}" placeholder="Search / playlist / channel / video IDs">
      <input class="count" type="number" min="1" max="100" value="${s.max_items || 20}" title="Max items">
      <input class="region" value="${esc(s.region_code || '')}" placeholder="Region (e.g. IN)">
      <label class="visible"><input type="checkbox" class="enabled" ${s.visible !== false ? 'checked' : ''}> Visible</label>
      <button class="up" ${i===0?'disabled':''}>↑</button>
      <button class="down" ${i===state.sections.length-1?'disabled':''}>↓</button>
    ...
    subtitle.oninput=e=>s.subtitle=e.target.value;
    source.onchange=e=>s.source_type=e.target.value;
    value.oninput=e=>s.source_value=e.target.value;
    count.oninput=e=>s.max_items=Math.max(1, Math.min(100, Number(e.target.value)||20));
    region.oninput=e=>s.region_code=e.target.value.toUpperCase();
    enabled.onchange=e=>s.visible=e.target.checked;
    d.querySelector('.up').onclick=()=>move(i,-1);
    d.querySelector('.down').onclick=()=>move(i,1);
    d.querySelector('.delete').onclick=()=>{ state.sections.splice(i,1); render(); };
    $('sections').appendChild(d);
  });
}

function move(i,delta){
  const j=i+delta; if(j<0 || j>=state.sections.length) return;
...
async function publish(){
  ...
  try {
    ...
    state.sections=rows;
    render();
    status(`Published successfully. ${rows.length} section${rows.length===1?'':'s'} are now live.`);
  } catch(e){ status(`Publish failed: ${e.message || e}`, true); }
...
supabase.auth.onAuthStateChange((_event)=>setTimeout(load,0));
load();

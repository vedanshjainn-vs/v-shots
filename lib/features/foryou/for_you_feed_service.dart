...
import '../../core/recommendation/music_discovery_engine.dart';
...
class ForYouFeedService {
  ForYouFeedService({
    ...
    _discoveryEngine = MusicDiscoveryEngine(repository: _repository);
  }

  final MusicRepository _repository;
  late final MusicDiscoveryEngine _discoveryEngine;
  ...
  Future<List<Map<String, dynamic>>> fetchNextBatch({
    ...
    try {
      // V4 Discovery: provider-native recommendations + trending + taste
      // profile + exploration, then V Shots ranking/diversity/seen memory.
      // This replaces the old single-query/random-roll path only for the
      // generic For You feed. Category/query feeds below remain untouched.
      return await _discoveryEngine.generate(
        excludeIds: excludeIds,
        count: count,
        moods: activeMood == null ? const [] : [activeMood!],
      );
    } catch (e) {
      debugPrint('[ForYouFeedService] V4 discovery failed: $e');
      // Hard fallback: preserve the existing paginated discovery path.

...
def patch_generator() -> None:
    ...
    path.write_text(path.read_text().replace('..shuffle();', '..sort();'))
...
def patch_home() -> None:
    ...
    new = """    // Only history-dependent shelves are gated. Broad shelves stay visible\n    // on cold start, but the V2 ranker gives them zero fabricated taste boost.\n    final profileSignals = TasteProfileBuilder().build().totalSignalCount;\n    if (shelf.onlyWhenPersonalized && (!hasPersonalization || profileSignals < 3)) {\n"""
    if old not in text:
        raise RuntimeError('Advanced recommendation patch: home personalization gate missing')
